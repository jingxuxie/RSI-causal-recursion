...
\begin{abstract}
Increasing task performance does not by itself establish that a system has improved its improvement mechanism, or that recursively inheriting that mechanism was beneficial. We separate transferable mechanism gain from a comparator-specific \emph{causal recursion dividend}: downstream improvement capability after recursive development minus that after resource-matched fixed-author development. We characterize nonidentification from native performance-and-lineage summaries, and formulate fork-and-transplant audits that identify the two effects under explicit interface assumptions. For imperfect transplants, we derive sharp finite-state identification intervals under rectangular transition uncertainty, distinguish shared-history uncertainty from independent branch uncertainty, and prove an endpoint-audit detection boundary. With total contrast distortion bounded by $b$, uniformly detecting a true dividend $\gamma$ is impossible when $\gamma\leq2b$; above this boundary the minimax sample size scales as $(\gamma-2b)^{-2}\log(1/\delta)$. Development-level confidence bounds account for dependent descendants. Exact finite-state experiments exhibit positive, zero, and negative inheritance effects. In an independent Boolean-program repair study with 4,096 development runs per task family, recursive and fixed-author development both improve the improver, while all three recursion-dividend intervals fall within a prespecified one-percentage-point equivalence band. These conclusions concern a bounded rule-table search system, not general-purpose AI. The framework supports explicit claims about recursive inheritance without equating ordinary progress with improvement of the improver.
\end{abstract}
...
Our experiments are deliberately small and interpretable. They do not validate broad transfer, language-model behavior, or unrestricted recursive improvement. The most informative program result is a separation: learning a better improver does not imply a measurable benefit from recursively inheriting authorship. The paper's tools are intended to make such distinctions auditable. Progress, mechanism improvement, and recursive advantage should be reported as different claims, with explicit intervention assumptions and uncertainty appropriate to the development process.
